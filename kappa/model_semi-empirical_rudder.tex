...
The semi-empirical rudder model was proposed in Paper \ref{pap:physics}  as a lifting line model similar to 	extcite{kjellberg_sailing_2023}, 	extcite{matusiak_dynamics_2021}, and 	extcite{hughes_tempest_2011} that is primarily based on the rudder wind tunnel tests conducted by 	extcite{whicker_free-stream_1958}. The surge and sway forces are expressed as rudder lift $L_R$ and rudder drag $D_R$, which are projected on the ship through the rudder inflow angle $\alpha_f$ (see \autoref{eq:X_R_semiempirical}, \autoref{eq:Y_R_semiempirical}, and \autoref{fig:inflow}).
