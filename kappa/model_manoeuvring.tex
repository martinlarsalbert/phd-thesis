...
\end{equation}
$\mathbf{M}$ and $\mathbf{C}$ are calculated according to \textcite{fossen_handbook_2011} as shown in \autoref{eq:M_expanded} and \autoref{eq:C_expanded}. 
\begin{equation}
    \label{eq:M_expanded}
...
\end{equation}
\mynote{The yawing moment $N$ has the so called Munk moment \cite{fossen_handbook_2011}:
$$
u v \left(- X_{\dot{u}} + Y_{\dot{v}}\right)
$$
In many other manoeuvring models such as the Abkowitz \cite{abkowitz_ship_1964} or MMG model \cite{yasukawa_introduction_2015}, the Munk moment is not explicitly included in the equations. The Munk moment contribution will instead be included in some of the hull coefficients, typically $N_V$ or ideally $N_{uv}$ if such a coefficient exist in the model.}
\newline
\vspace{0.3cm}
