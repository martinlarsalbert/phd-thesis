...
In this procedure, a VMM is used to solve the reversed manoeuvring problem, such as predicting unknown forces from known ship manoeuverability. The hydrodynamic derivatives in the VMM can be identified with regression of the force polynomials on forces predicted with inverse dynamics, as seen in Fig.\ref{fig:greyvmm}. The Ordinary Least Square (OLS) method regresses the hydrodynamic derivatives. 
...
\begin{figure}[H]
    ...
    \draw [arrow] (X_D) -- (black-box);
    \draw [arrow] (Y_D)  -- (black-box);
    \draw [arrow] (N_D)  -- (black-box);
