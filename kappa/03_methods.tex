...
\node (train)[rectangle,
    ...
    text = black,
    minimum width=5cm,
    fill = yellow, below of dataset] (r){TRAIN};

\node (validation)[rectangle,
    draw,
    text = blue,
    minimum width=5cm,
    fill = white, right of=train, xshift=4.0cm]{VALIDATION};

\node (test)[rectangle,
    draw,
    text = white,
    minimum width=5cm,
    fill = green, right of=validation, xshift=4.0cm]{TEST};
    
\node (train_multiple)[rectangle,
    draw,
    text = black,
    draw = none,
    fill = none, below of=train]{Train multiple models};
    
\node(validate_models)[rectangle,
    draw,
    text = black,
    draw = none,
    fill = none, below of=validation]{Validate models};
    
\node(evaluate_models)[rectangle,
    draw,
    text = black,
    draw = none,
    fill = none, below of=test]{Evaluate models};

\draw[shorten <=0.3cm,->] (train) -- (train_multiple)
\draw[shorten <=0.3cm,->] (validation) -- (validate_models)
\draw[shorten <=0.3cm,->] (test) -- (evaluate_models)
    
\end{tikzpicture}
\label{fig:model_development_provess}
\caption{Caption}
\end{figure}
